...
function Cart() {
  ...
  function handleIncrement(item) {
    dispatch(update({ ...item, count: item.count + 1 }));
  }
    ...
    const CustomConfirmToast = () => (
      <div>
        ...
        <div className="flex gap-2 mt-2">
          <button onClick={() => { dispatch(remove(item)); toast.dismiss(); notify("Mahsulot o‘chirildi!", "success"); }} className="bg-[#00C17B] px-3 py-1 rounded text-white" > Ha </button>
          ...
          <div className="flex flex-col justify-center border-[#c1c6cf] dark:border-[#484A4E] mb-20 p-4 border-t-[1px] w-full">
            {cartItems.map((item) => (
              <div key={`${item.id}-${item.color}-${item.size}`} className="flex items-center border-[#c1c6cf] dark:border-gray-700 my-4 mb-4 pb-4 border-b">
                <img src={item.product.product_images[0].image} alt={item.product.name} className="shadow-md rounded-md w-[75px] h-[75px] object-cover" />
                <div className="flex justify-between items-start w-full">
                  <div className="flex-grow ml-4">
                    ...
                    <div className="flex items-center gap-4">
                      <p className="">Hajmi: <span className="uppercase">{item.size}</span></p>
                      <p className="flex items-center gap-1">Rangi: {item.color}</p>
                    </div>
                    <div className="flex items-center mt-2">
                      <button onClick={() => handleDecrement(item)} className="flex justify-center items-center bg-[#e7e7e7] dark:bg-gray-700 rounded-l w-5 h-5 text-sm dark:text-white"> - </button>
                      <span className="flex justify-center items-center bg-[#cdcdcd] dark:bg-gray-600 w-14 h-5 text-[12px] dark:text-white">{item.count} dona</span>
                  ...
                  <button disabled={isDisable} onClick={() => handleRemove(item)} className="bg-white mt-2 rounded-full underline"> <img src={Delete} className="w-6 h-6" /> </button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bottom-0 fixed bg-[#EDEDED] dark:bg-[#121212] mx-auto p-4 rounded-tl-[30px] rounded-tr-[30px] w-full max-w-[600px]">
          <div className="mb-4 text-sm dark:text-gray-300">
            <div className="flex justify-between mb-4">
              ...
              <span>{cartItems.reduce((acc, item) => acc + item.product.discount_price * item.count, 0)} UZS</span>
            </div>
            <hr className="border-[#484A4E]" />
            <div className="flex justify-between mt-2">
              <span>Yetkazib berish</span>
              <span className="text-[#00C17B]">Bepul</span>
            </div>
          </div>
          ...
          <button onClick={handleClick} className="bg-black dark:bg-white py-2 rounded w-full font-medium text-center text-white dark:text-black transition"> Qabul qilaman </button>
        </div>
      </div>
    </>
  );
